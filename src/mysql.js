 ...
 */
const query = async function (pool, sqls, args) {
  assertArgs(sqls, args)
  let data = []
  for (let i = 0, l = sqls.length; i < l; i++) {
    // 注意这里的args=null的问题
    data.push(await execSingleQuery(pool, sqls[i], getArg(i, args)))
  }
  return data
}
 ...
 */
const execSingleQuery = function (pool, sql, arg) {
 ...
 */
const assertArgs = function (sqls, args) {
  if (!_.isArray(sqls)) {
 ...
 */
const getArg = function (index, args) {
  if (args === null) {
    return null
  }
